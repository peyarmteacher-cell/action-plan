...
CREATE TABLE `schools` (
  ...
  `website` VARCHAR(255) DEFAULT NULL COMMENT 'เว็บไซต์โรงเรียน',
  `vision` TEXT DEFAULT NULL COMMENT 'วิสัยทัศน์',
  `mission` TEXT DEFAULT NULL COMMENT 'พันธกิจ',
  `motto` VARCHAR(255) DEFAULT NULL COMMENT 'คำขวัญ',
  `school_colors` VARCHAR(100) DEFAULT NULL COMMENT 'สีประจำโรงเรียน',
  `logo_url` TEXT DEFAULT NULL COMMENT 'โลโก้โรงเรียน',
  `allow_project_submission` TINYINT(1) NOT NULL DEFAULT 1 COMMENT 'สวิตช์: 1=เปิดรับการเสนอโครงการ, 0=ปิดรับ',
...
CREATE TABLE `users` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `school_id` INT UNSIGNED NOT NULL,
  `username` VARCHAR(50) NOT NULL COMMENT 'ชื่อผู้ใช้สำหรับเข้าสู่ระบบ (รหัส SMIS สำหรับ Admin หรือ เลขบัตร ปชช. 13 หลัก สำหรับครู)',
  `citizen_id` VARCHAR(13) DEFAULT NULL COMMENT 'หมายเลขประจำตัวประชาชน 13 หลัก',
  ...
  `must_change_password` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '1=ต้องเปลี่ยนรหัสผ่านในการเข้าใช้งานครั้งแรก, 0=เปลี่ยนแล้ว',
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE `projects` (
  ...
  `school_id` INT UNSIGNED NOT NULL,
  `fiscal_year_id` INT UNSIGNED NOT NULL,
  `semester` TINYINT UNSIGNED DEFAULT 1 COMMENT 'ภาคเรียน: 1 หรือ 2',
  ...
  `approval_status` ENUM('draft', 'pending', 'approved', 'rejected', 'revision') NOT NULL DEFAULT 'draft' COMMENT 'สถานะอนุมัติ: draft, pending, approved, rejected, revision',
  `review_comment` TEXT DEFAULT NULL COMMENT 'ข้อคิดเห็น/เหตุผลประกอบการอนุมัติหรือส่งกลับแก้ไข',
